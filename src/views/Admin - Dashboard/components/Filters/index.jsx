        ...
        filtersValues.length && filtersValues.map((f, i) => {
          ...
          return (
            <div key={i}>
              ...
              <select className='block w-full px-4 py-3 text-base text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500' name={f.param} onChange={(e) => handleFilter(e, f.param)}>
                <option value='' selected={!isFilterActive}>- - -</option>
                <option value={f.valueOne ? f.valueOne : f.optionAZ ? f.optionAZ : f.optionOne}>{f.optionOne}</option>
                <option value={f.valueTwo ? f.valueTwo : f.optionZA ? f.optionZA : f.optionTwo}>{f.optionTwo}</option>
                {
                  f.optionThree && <option value={f.valueThree ? f.valueThree : f.optionThree}>{f.optionThree}</option>
                }
              </select>
            </div>
          );
