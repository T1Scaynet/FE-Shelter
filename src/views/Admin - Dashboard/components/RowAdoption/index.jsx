        ...
        info.length
          ? info.map((data, i) => (
            <div key={i} className={`${i % 2 === 0 ? 'bg-white' : 'bg-[#f8f8f8]'} grid grid-cols-4 `}>
              ...
              <div className='w-[22.4rem] md:w-[71.825rem]'>
                <Collapse in={showInput[data._id]}>
                  <div className='w-full h-[18rem] border-x-2 px-3 py-3 border-b-2 border-[#f0f0f0]'>
                    <h3 className='border-b-2 border-black font-bold text-xl text-center mb-3'>Datos Pesonales</h3>
                    <div className='flex flex-row w-full justify-between'>
                      <div>
                        <b className='border-b-2 border-black'>ID</b>
                        <p className='mt-3'><b>ID_Usuario: </b>{data.idUser}</p>
                        <p className='mt-[0.25rem]'><b>ID_Usuario: </b>{data.idUser}</p>
                      </div>
                      <div>
                        <b className='border-b-2 border-black'>INFO</b>
                        <p className='mt-3'><b>Nombre: </b>{data.name}</p>
                        <p className='mt-[0.25rem]'><b>Apellido: </b>{data.lastName}</p>
                        <p className='mt-[0.25rem]'><b>Dirección: </b>{data.address}</p>
                        <p className='mt-[0.25rem]'><b>E-mail: </b>{data.email}</p>
                        <p className='mt-[0.25rem]'><b>Teléfono: </b>{data.phone}</p>
                        <p className='mt-[0.25rem]'><b>Fecha de solicitud: </b>{data.createAt}</p>
                      </div>
                      <div>
                        <b className='border-b-2 border-black'>DETALLES DEL HOGAR</b>
                        <p className='mt-3'><b>Otras mascotas: </b>{data.otherPets ? 'Si' : 'No'}</p>
                        <p className='mt-[0.25rem]'><b>Patio: </b>{data.garden ? 'Si' : 'No'}</p>
                        <p className='mt-[0.25rem]'><b>Chicos: </b>{data.children ? 'Si' : 'No'}</p>
                      </div>
                    </div>
                  </div>
                </Collapse>
